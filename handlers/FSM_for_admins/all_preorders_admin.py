...
class all_preorders_admins_fsm(StatesGroup):
    category = State()
...
async def fsm_start(message: types.Message):
    if message.from_user.id in Admins:
        await all_preorders_admins_fsm.category.set()
        await message.answer(f"Категория предзаказа?", reply_markup=buttons.CategoryButtonsAdmins)
    else:
        ...
        if message.text.startswith("/"):
            ...
            if preorders:
                if len(preorders) <= 5:
                    ...
                    if current_chunk < len(chunks) - 1:
                        ...
                        await all_preorders_admins_fsm.next()
            else:
            ...
            if preorders:
                ...
                if current_chunk < len(chunks) - 1:
                    ...
                    ShowMore.add(KeyboardButton('Отмена!'))
                    await message.answer("Показать еще?", reply_markup=ShowMore)
                    await all_preorders_admins_fsm.more_preorders.set()
                else:
                    await state.finish()
                    await message.answer(f"Это все предзаказы из категории: {category}",
                                         reply_markup=buttons.StartAdmin)
...
def register_all_preorders_admins(dp: Dispatcher):
    dp.register_message_handler(cancel_reg, Text(equals="Отмена!", ignore_case=True), state="*")
    dp.register_message_handler(fsm_start, commands=["Предзаказы", 'all_preorders_admin'])
    dp.register_message_handler(load_category, state=all_preorders_admins_fsm.category)
    for category in ["Обувь", "Нижнее_белье", "Акссесуары", "Верхняя_одежда", "Штаны"]:
        dp.register_message_handler(load_more_preorders,
                                    ...
                                    state=all_preorders_admins_fsm.more_preorders)
